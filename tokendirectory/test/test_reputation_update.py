...
class UpdateReputationTest(AsyncHandlerTest):
    ...
    async def test_update_reputation(self):
        ...
        async with self.pool.acquire() as con:
            await con.execute("INSERT INTO apps (token_id) VALUES ($1)", TEST_PAYMENT_ADDRESS)
        ...
        async with self.pool.acquire() as con:
            row = await con.fetchrow("SELECT * FROM apps WHERE token_id = $1", TEST_PAYMENT_ADDRESS)
